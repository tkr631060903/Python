...
class AutoTest():
    ...
    def U_InitDevices(self):
        ...
        sleep(0.1)
        self.L_ref = ATS.myLCR.get_measure_val("LSRD", 200000, 0.5)[0]
        self.R_ref = ATS.myLCR.get_measure_val("LSRD", 100, 0.5)[1]
    ...
    def U_GetDatas(self, mode=None):
        ...
        try:
            ...
            self.meas_out[1] = ATS.myLCR.get_measure_val("LSRD", 100, 0.5)[1]
            self.meas_out[0] -= self.L_ref
            self.meas_out[1] -= self.R_ref
            self.meas_out[0] = abs(self.meas_out[0])
            self.meas_out[1] = abs(self.meas_out[1])
            for i in range(2):
