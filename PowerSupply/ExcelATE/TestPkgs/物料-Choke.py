...
class AutoTest():
    ...
    def U_GetDatas(self, mode=None):
        ...
        try:
            # Choke直流阻抗：meas_out[0]；
            Volt_test = ATS.myDMM.get_dc_val()
            if Volt_test > self.u_wkmode["Vin"] - 0.2:  # Choke开路
                self.meas_out[0 + 2 * mode] = "ERROR"
            else:
                Curr_test = ATS.getDCPowerMeasCurr()
                self.meas_out[0 + 2 * mode] = round(Volt_test / Curr_test, 3)
            self.meas_out[1 + 2 * mode] = round(Curr_test, 3)
