    ...
    def U_GetDatas(self, mode=None):
        ...
        try:
            ...
            while clock() - stclk < 310:
                ...
                if clock() - stclk > i:
                    if i % 15 == 0:
                        ATS.setDCPowerVolt(val_cof * self.u_wkmode['Vin'])
                    elif i % 15 == 5:
                        ATS.setDCPowerVolt(self.u_wkmode['Vin'])
                    elif i % 15 == 14:
                        if (val_cof >= 0.05):
                            ...
                            val_cof = 1
                    print("[process]" + "当前降落系数:" + str(round(val_cof, 3)) +
                          " 预计" + str(int(340 - i)) + "S后完成")
                    i += 1
            while clock() - stclk < 340:
                j += 1
                val.append(ATS.getDCPowerMeasVolt())
                cur.append(ATS.getDCPowerMeasCurr())
                xalias.append(j)
                if clock() - stclk > i:
                    print("[process]" + "预计" + str(int(340 - i)) + "S后完成")
                    i += 1
            for index in range(len(xalias)):
                xalias[index] = round(xalias[index] * (340 / j), 3)
            html_name = datetime.now().strftime('%Y%m%d-%H%M%S') + '-电压骤降复位'
