...
class AutoTest():
    ...
    def __init__(self):
        ...
        print("[process]" + "@输入电压:" + str(self.u_wkmode['Vin']) + "V Iout:" +
    ...
    def U_InitDevices(self):
        ...
        try:
            ...
            ATS.myScope.set_channel(2, "ON", "AC", 1, 3, 10)
            ATS.myScope.set_timebase(0.001, 40)
            ATS.myScope.set_trigger(1, "NORM", "FALL",
                                    self.u_wkmode['Iout'] * 0.5)
            ...
            ATS.myEload.set_dynamic("CONT", self.u_wkmode['Iout'], 0.001,
                                    self.u_wkmode['Iout'] * 0.3, 0.0001)
            ATS.myScope.set_run()
    ...
    def U_GetDatas(self, mode=None):
        ...
        try:
            vrip = 0
            iout = self.u_wkmode['Iout']
            while vrip < 1000:
                iout = round(iout + 0.05, 3)
                st_clk = clock()
                i = 0
                ATS.myEload.set_dynamic("CONT", iout, 0.005,
                                        self.u_wkmode['Iout'] * 0.3, 0.0001)
                ATS.myScope.set_run()
                while clock() - st_clk < 5:
                    if clock() - st_clk > i:
                        i += 1
                        print(
                            '\u001b[0K' + "[process]" + "@正向测试 Iout:" +
                            str(iout) + "A 预计" + str(6 - i) + "S后完成",
                            end='\r')
                        if i == 3:
                ...
                vrip = self.meas_out[1] - self.meas_out[2]
                print('\u001b[0K' + "[process]" + "@正向测试 Iout:" + str(iout) +
                      "A 完成")
            self.meas_out[3] = ATS.getScopeScreen(
                self.u_wkmode['Path'] + '/Image', "-异常波形")

            while vrip > 1000:
                iout = round(iout - 0.01, 3)
                ATS.myEload.set_dynamic("CONT", iout, 0.005,
                                        self.u_wkmode['Iout'] * 0.3, 0.0001)
                st_clk = clock()
                i = 0
                while clock() - st_clk < 300:
                    if clock() - st_clk > i:
                        i += 1
                        print(
                            '\u001b[0K' + "[process]" + "@反向测试 Iout:" +
                            str(iout) + "A 预计" + str(301 - i) + "S后完成",
                            end='\r')
                        if i % 6 == 1:
                            ATS.myScope.set_run()
                        if i % 6 == 3:
                            ATS.myScope.set_stop()
                        if i % 6 == 5:
                            float_tmp = ATS.myScope.get_measure_val(
                                2, "MAX") * 1000.0
                            if float_tmp < 10000:
                                self.meas_out[1] = round(float_tmp, 3)
                            float_tmp = ATS.myScope.get_measure_val(
                            ...
                            if vrip > 1000:
                                break
                print('\u001b[0K' + "[process]" + "@反向测试 Iout:" + str(iout) +
                      "A 完成")
            self.meas_out[0] = iout
            self.meas_out[4] = ATS.getScopeScreen(
                self.u_wkmode['Path'] + '/Image', "-正常波形")
        except Exception as err:
