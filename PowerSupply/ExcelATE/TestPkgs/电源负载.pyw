...
from PySide2.QtWidgets import QApplication, QLabel, QFileDialog, QScrollArea, QSizePolicy, QInputDialog, QMessageBox
from PySide2.QtCore import QFile, QObject, Slot, QTimer, QSize, Qt
from PySide2.QtGui import QIcon, QKeySequence, QClipboard, QDoubleValidator
...
class UI(QObject):
    def __init__(self, ui, path):
        ...
        self.use_devices = ['直流电源', '电子负载', '示波器']
    ...
    def createImageLable(self):
        ...
        ui.layoutScope.addWidget(self.scrollArea)
        ...
        validator_val = QDoubleValidator(0.0, 30.0, 3)
        ui.lineEditPowerVal.validator.errorinput.connect(
            lambda: self.messageBox_show("请输入0.000-30.000内的浮点数!"))
        ui.lineEditPowerVal.setValidator(validator_val)
        ui.lineEditPowerVal.editingFinished.connect(
            lambda: self.lineEdit_editingFinished(ui.lineEditPowerVal))
        ui.lineEditPowerCurr.editingFinished.connect(
            lambda: self.lineEdit_editingFinished(ui.lineEditPowerCurr))
        ui.lineEditLoadCurr.editingFinished.connect(
            lambda: self.lineEdit_editingFinished(ui.lineEditLoadCurr))
    ...
    @Slot()
    def messageBox_show(self, text):
        print(text)
        self.msgBox.setText(text)
        self.msgBox.exec_()

    @Slot()
    def lineEdit_editingFinished(self, sender):
        # action = self.sender()
        if sender is ui.lineEditPowerVal:
            ATS.setDCPowerVolt(sender.text())
            print(sender.text())
        # if action == "":
        #     pass
        # else:
...
if __name__ == "__main__":
    ...
    ui_file = QFile(thisfiledir + "/ui/tools.ui")
